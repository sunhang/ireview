...
//    canvas.drawColor(Color.fromARGB(255, 255, 255, 255), BlendMode.srcIn);
    ...
    for (double i = 0; i < size.width / 2; i += 40) {
      canvas.drawLine(Offset(size.width / 2 + i, size.height / 2), Offset(size.width / 2 + i, size.height / 2 - 10), _paint);
      canvas.drawLine(Offset(size.width / 2 - i, size.height / 2), Offset(size.width / 2 - i, size.height / 2 - 10), _paint);
    }

    for (double i = 0; i < size.height / 2; i += 40) {
      canvas.drawLine(Offset(size.width / 2, size.height / 2 + i), Offset(size.width / 2 + 10, size.height / 2 + i), _paint);
      canvas.drawLine(Offset(size.width / 2, size.height / 2 - i), Offset(size.width / 2 + 10, size.height / 2 - i), _paint);
    }

    final arrowSize = 15.0;
    // 画X轴箭头
    canvas.drawLine(Offset(size.width, size.height / 2), Offset(size.width - arrowSize, size.height / 2 - arrowSize), _paint);
    canvas.drawLine(Offset(size.width, size.height / 2), Offset(size.width - arrowSize, size.height / 2 + arrowSize), _paint);

    // 画Y轴箭头
    canvas.drawLine(Offset(size.width / 2, 0), Offset(size.width / 2 - arrowSize, arrowSize), _paint);
    canvas.drawLine(Offset(size.width / 2, 0), Offset(size.width / 2 + arrowSize, arrowSize), _paint);
    ...
    TextPainter tpX = new TextPainter(
        text: spanX, textDirection: TextDirection.ltr);
    tpX.layout();
    tpX.paint(canvas, new Offset(size.width - tpX.width, size.height / 2 + arrowSize));

    // 绘制Y
    TextSpan spanY = new TextSpan(
        style: new TextStyle(color: Colors.black, fontSize: 20.0), text: "Y");
    TextPainter tpY = new TextPainter(
        text: spanY, textDirection: TextDirection.ltr);
    tpY.layout();
